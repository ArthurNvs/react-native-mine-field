...
const isAreaSafe = (board, row, column) => {
    const checkSafety = (result, neighbor) => result && !neighbor.mined
    return getNeighbours(board, row, column).reduce(checkSafety, true)
}
...
const openField = (board, row, column) => {
    ...
    if (!field.opened) {
        ...
        if (field.mined) {
            field.exploded = true
        } else if (isAreaSafe(board, row, column)) {
...
const winGame = board => fields(board).filter(pending).length === 0
